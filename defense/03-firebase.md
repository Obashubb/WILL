...
- Sign-up and sign-in are working through `AuthService` + `AuthController`.
- The app gates the home shell behind an `AuthGate` widget that watches `FirebaseAuth.instance.authStateChanges()`.

**Manual steps you still need to do in the Firebase console** before the build can actually sign anyone in:
1. Turn on the **Authentication → Email/Password** provider.
2. Create the **Cloud Firestore** database (start in test mode for development).
...
- `lib/view/auth/auth_controller.dart` — GetX controller that tracks the current user and exposes sign-in / sign-up / sign-out.
- `lib/view/auth/auth_gate.dart` — picks between the login screen and the home shell based on auth state.
- `lib/view/auth/login_screen.dart` and `lib/view/auth/signup_screen.dart` — the two auth screens.
- `lib/services/sync_service.dart` — *planned* wrapper around `cloud_firestore` (created in Phase 4).
